...
- [x] 6.1 Docker 启动，验证 preset 自动创建且 `zhipu` 为默认激活 LLM
- [x] 6.2 前端添加自定义 provider（如 DeepSeek），测试连接通过
- [x] 6.3 切换 provider，发送消息验证使用新模型
- [x] 6.4 验证 api_key 在 DB 中为密文、API 响应中脱敏
- [x] 6.5 验证删除自定义 provider、无法删除 preset
- [x] 6.6 验证 Embedding provider 切换后知识库检索正常
