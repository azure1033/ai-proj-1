# chat-history-persistence (delta)
...
On first startup with MySQL enabled, the system SHALL automatically migrate existing `chat_history.json` data into MySQL under a dedicated "历史记录" session. Migration logic SHALL be in the startup event handler in `main.py`, moved from the inline function to a dedicated migration utility.
...
None — all existing behavioral requirements remain. Only the import paths and file organization change.
