...
系统 SHALL 使用 RecursiveCharacterTextSplitter 以中文标点为优先分隔符进行文档分块。（此要求不变，保留原有行为）
...
系统 SHALL 通过 `provider_manager.get_active_embedding()` 获取嵌入模型实例，将文本块转换为嵌入向量。（此要求保留原有行为，仅统一嵌入模型获取方式）
...
### Requirement: 模块级 DOCUMENTS 全局列表

**Reason**: 模块级全局变量 `DOCUMENTS: list[dict] = []` 不是线程安全的，且无法跨请求持久化。替换为会话存储系统支持的文档追踪。

**Migration**: 文档元数据现在通过 `session_store` 的文档注册表或数据库的 sessions 表追踪。`GET /documents` 端点现在需要 `session_id` 参数来确定作用域。
